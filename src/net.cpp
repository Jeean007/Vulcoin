...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    int64_t nPingUvlcWait = 0;
    if ((0 != nPingNonceSent) && (0 != nPingUvlcStart)) {
        ...
        BOOST_FOREACH (CNode* pnode, vNodesCopy) {
            ...
            if (nTime - pnode->nTimeConnected > 60) {
                if (pnode->nLastRecv == 0 || pnode->nLastSend == 0) {
                    ...
                    pnode->fDisconnect = true;
                } else if (pnode->nPingNonceSent && pnode->nPingUvlcStart + TIMEOUT_INTERVAL * 1000000 < GetTimeMicros()) {
                    LogPrintf("ping timeout: %fs\n", 0.000001 * (GetTimeMicros() - pnode->nPingUvlcStart));
                    pnode->fDisconnect = true;
                }
            }
        }
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        // Do this here so we don't have to critvlct vNodes inside mapAddresses critvlct.
        int nOutbound = 0;
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Vulcoin Core is probably already running."), addrBind.ToString());
        else
