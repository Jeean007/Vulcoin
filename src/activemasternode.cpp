...
//
// Bootup the Masternode, look for a VLC collateral input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        //     if (service.GetPort() != 22300) {
        //         notCapableReason = strprintf("Invalid port: %u - only 22300 is supported on mainnet.", service.GetPort());
        //         LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //         return;
        //     }
        // } else if (service.GetPort() == 22300) {
        //     notCapableReason = strprintf("Invalid port: %u - 22300 is only supported on mainnet.", service.GetPort());
        //     LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //     return;
        // }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    // if (Params().NetworkID() == CBaseChainParams::MAIN) {
    //     if (service.GetPort() != 22300) {
    //         errorMessage = strprintf("Invalid port %u for masternode %s - only 22300 is supported on mainnet.", service.GetPort(), strService);
    //         LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //         return false;
    //     }
    // } else if (service.GetPort() == 22300) {
    //     errorMessage = strprintf("Invalid port %u for masternode %s - 22300 is only supported on mainnet.", service.GetPort(), strService);
    //     LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //     return false;
    // }
...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& vlcretKey)
{
    return GetMasterNodeVin(vin, pubkey, vlcretKey, "", "");
}

bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& vlcretKey, std::string strTxHash, std::string strOutputIndex)
{
    ...
    return GetVinFromOutput(*selectedOutput, vin, pubkey, vlcretKey);
}


// Extract Masternode vin information from output
bool CActiveMasternode::GetVinFromOutput(COutput out, CTxIn& vin, CPubKey& pubkey, CKey& vlcretKey)
{
    CScript pubScript;
    ...
    if (!pwalletMain->GetKey(keyID, vlcretKey)) {
        LogPrintf("CActiveMasternode::GetMasterNodeVin - Private key for address is not known\n");
        return false;
    }

    pubkey = vlcretKey.GetPubKey();
    return true;
}
