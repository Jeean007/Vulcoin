...
unsigned int nStakeMinAge = 15 * 24 * 60 * 60; //15 days
unsigned int nStakeMaxAge = 90 * 24 * 60 * 60; //90 days
unsigned int nModifierInterval = 60 * 60; // time to elapse before new modifier is computed
...
const string strMessageMagic = "vulcoin Signed Message:\n";
...
{
    int64_t nRewardCoinYear = 15 * CENT; // 1 YEAR

            if(nBestHeight > 175200) // 2 YEAR
            {
				nRewardCoinYear = 10 * CENT;
            }

            else if(nBestHeight > 350400)  // 3 YEAR
            {
				nRewardCoinYear = 8 * CENT;
            }  

			else if(nBestHeight > 525600)// 4 YEAR
            {
				nRewardCoinYear = 7 * CENT;
            }
			else if(nBestHeight > 700800)// NEXT YEARs
            {
...
static const int64_t nTargetTimespan_v1 = 3 * 15 * 60;
static const int64_t nTargetTimespan_v2 = 3 * 15 * 60;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "vulcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Vulcoin new born!!!!";
        CTransaction txNew;
        txNew.nTime = 1549454049;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1549454049;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 237941 : 237941;
        ...
        assert(block.hashMerkleRoot == uint256("0xd14c8ae26dc96c3fe83cc6ab4367a863a0aa656c8cb224c9b1a1143d9a64c65e"));
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // vlc: send latest proof-of-work block to allow the
