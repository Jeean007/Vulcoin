...
public:
    ...
    // Time (in uvlc) the last ping was sent, or 0 if no ping was ever sent.
    int64_t nPingUvlcStart;
    // Last measured round-trip time.
    int64_t nPingUvlcTime;
    ...
    void PushAddress(const CAddress& addr)
    {
        ...
        if (addr.IsValid() && !setAddrKnown.count(addr)) {
            if (vAddrToSend.size() >= MAX_ADDR_TO_SEND) {
                vAddrToSend[invlcure_rand() % vAddrToSend.size()] = addr;
            } else {
