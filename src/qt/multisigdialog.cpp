...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet, string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw runtime_error("Not enough VLC provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
           ...
           feeStringRet = strprintf("%d",((double)fee)/COIN).c_str();
            ...
            throw runtime_error("Not enough VLC provided to cover fee");
        }
    ...
    try{
        ...
        if(fGivenKeys){
            for(int i = 0; i < keyList->count(); i++){
                ...
                if (!vchVlcret.SetString(key->text().toStdString()))
                    throw runtime_error("Invalid private key");
                CKey cKey = vchVlcret.GetKey();
                if (!cKey.IsValid())
