...
/**
 * Fees smaller than this (in uvulcoin) are considered zero fee (for transaction creation)
...
CPubKey CWallet::GenerateNewKey()
{
    ...
    RandAddSeedPerfmon();
    CKey vlcret;
    vlcret.MakeNewKey(fCompressed);

    // Compressed public keys were introduced in version 0.6.0
    if (fCompressed)
        SetMinVersion(FEATURE_COMPRPUBKEY);

    CPubKey pubkey = vlcret.GetPubKey();
    assert(vlcret.VerifyPubKey(pubkey));
    ...
    if (!AddKeyPubKey(vlcret, pubkey))
        throw std::runtime_error("CWallet::GenerateNewKey() : AddKey failed");
    return pubkey;
}

bool CWallet::AddKeyPubKey(const CKey& vlcret, const CPubKey& pubkey)
{
    AssertLockHeld(cs_wallet); // mapKeyMetadata
    if (!CCryptoKeyStore::AddKeyPubKey(vlcret, pubkey))
        return false;
    ...
    if (!fFileBacked)
        return true;
    if (!IsCrypted()) {
        return CWalletDB(strWalletFile).WriteKey(pubkey, vlcret.GetPrivKey(), mapKeyMetadata[pubkey.GetID()]);
    }
    return true;
}

bool CWallet::AddCryptedKey(const CPubKey& vchPubKey,
    const vector<unsigned char>& vchCryptedVlcret)
{
    if (!CCryptoKeyStore::AddCryptedKey(vchPubKey, vchCryptedVlcret))
        return false;
    if (!fFileBacked)
        return true;
    {
        ...
        if (pwalletdbEncryption)
            ...
            return CWalletDB(strWalletFile).WriteCryptedKey(vchPubKey, vchCryptedVlcret, mapKeyMetadata[vchPubKey.GetID()]);
    }
    return false;
}
...
bool CWallet::LoadCryptedKey(const CPubKey& vchPubKey, const std::vector<unsigned char>& vchCryptedVlcret)
{
    return CCryptoKeyStore::AddCryptedKey(vchPubKey, vchCryptedVlcret);
}
...
bool CWallet::Unlock(const VlcureString& strWalletPassphrase, bool stakingOnly)
{
    VlcureString strWalletPassphraseFinal;
...
bool CWallet::ChangeWalletPassphrase(const VlcureString& strOldWalletPassphrase, const VlcureString& strNewWalletPassphrase)
{
    bool fWasLocked = IsLocked();
    VlcureString strOldWalletPassphraseFinal = strOldWalletPassphrase;
...
bool CWallet::EncryptWallet(const VlcureString& strWalletPassphrase)
{
    if (IsCrypted())
        return false;
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++) {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++) {
            for (unsigned int i = 0; i < vValue.size(); i++) {
                ...
                //the randomness serves no real vlcurity purpose but is just
                //needed to prevent degenerate behavior and it is important
                //that the rng is fast. We do not use a constant random sequence,
                //because there may be some privacy improvement by making
                //the selection random.
                if (nPass == 0 ? invlcure_rand() & 1 : !vfIncluded[i]) {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 VLC.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-vulcoin-address
                    CScript scriptChange;
...
{
    // The following split & combine thresholds are important to vlcurity
