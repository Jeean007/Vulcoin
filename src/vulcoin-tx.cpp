...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("Vulcoin Core vulcoin-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  vulcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded vulcoin transaction") + "\n" +
                               "  vulcoin-tx [options] -create [commands]   " + _("Create hex-encoded vulcoin transaction") + "\n" +
                               "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    for (unsigned int kidx = 0; kidx < keysObj.size(); kidx++) {
        ...
        bool fGood = vchVlcret.SetString(keysObj[kidx].getValStr());
        if (!fGood)
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded vulcoin transaction
