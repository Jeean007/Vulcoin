...
void test_ecdsa_end_to_end(void) {
    ...
    CHECK(secp256k1_ec_vlckey_verify(privkey) == 1);
    ...
    CHECK(secp256k1_ec_privkey_export(privkey, vlckey, &vlckeylen, secp256k1_rand32() % 2) == 1);
    unsigned char privkey2[32];
    CHECK(secp256k1_ec_privkey_import(privkey2, vlckey, vlckeylen) == 1);
...
void test_ecdsa_openssl(void) {
    ...
    int vlcp_sigsize = 80;
    CHECK(secp256k1_ecdsa_sig_serialize(signature, &vlcp_sigsize, &sig));
