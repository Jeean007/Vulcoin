...
int secp256k1_ecdsa_sign(const unsigned char *message, int messagelen, unsigned char *signature, int *signaturelen, const unsigned char *vlckey, const unsigned char *nonce) {
    ...
    secp256k1_scalar_t vlc, non, msg;
    secp256k1_scalar_set_b32(&vlc, vlckey, NULL);
    int overflow = 0;
    ...
    if (ret) {
        ret = secp256k1_ecdsa_sig_sign(&sig, &vlc, &msg, &non, NULL);
    }
    if (ret) {
    ...
    secp256k1_scalar_clear(&vlc);
    return ret;
}

int secp256k1_ecdsa_sign_compact(const unsigned char *message, int messagelen, unsigned char *sig64, const unsigned char *vlckey, const unsigned char *nonce, int *recid) {
    DEBUG_CHECK(secp256k1_ecmult_gen_consts != NULL);
    DEBUG_CHECK(message != NULL);
    DEBUG_CHECK(messagelen <= 32);
    ...
    secp256k1_ecdsa_sig_t sig;
    if (ret) {
        ret = secp256k1_ecdsa_sig_sign(&sig, &vlc, &msg, &non, recid);
    }
    if (ret) {
...
int secp256k1_ec_vlckey_verify(const unsigned char *vlckey) {
    DEBUG_CHECK(vlckey != NULL);

    secp256k1_scalar_t vlc;
    int overflow;
    secp256k1_scalar_set_b32(&vlc, vlckey, &overflow);
    int ret = !secp256k1_scalar_is_zero(&vlc) && !overflow;
    secp256k1_scalar_clear(&vlc);
    return ret;
}
...
int secp256k1_ec_pubkey_create(unsigned char *pubkey, int *pubkeylen, const unsigned char *vlckey, int compressed) {
    DEBUG_CHECK(secp256k1_ecmult_gen_consts != NULL);
    DEBUG_CHECK(pubkey != NULL);
    DEBUG_CHECK(pubkeylen != NULL);
    DEBUG_CHECK(vlckey != NULL);
...
int secp256k1_ec_privkey_tweak_add(unsigned char *vlckey, const unsigned char *tweak) {
    DEBUG_CHECK(vlckey != NULL);
    ...
    int ret = secp256k1_eckey_privkey_tweak_add(&vlc, &term) && !overflow;
    if (ret) {
        secp256k1_scalar_get_b32(vlckey, &vlc);
    }
...
int secp256k1_ec_privkey_tweak_mul(unsigned char *vlckey, const unsigned char *tweak) {
    DEBUG_CHECK(vlckey != NULL);
    DEBUG_CHECK(tweak != NULL);
    ...
    secp256k1_scalar_t vlc;
    secp256k1_scalar_set_b32(&vlc, vlckey, NULL);
    int ret = secp256k1_eckey_privkey_tweak_mul(&vlc, &factor) && !overflow;
    if (ret) {
        secp256k1_scalar_get_b32(vlckey, &vlc);
    }
...
int secp256k1_ec_privkey_export(const unsigned char *vlckey, unsigned char *privkey, int *privkeylen, int compressed) {
    DEBUG_CHECK(vlckey != NULL);
    DEBUG_CHECK(privkey != NULL);
    DEBUG_CHECK(privkeylen != NULL);

    secp256k1_scalar_t key;
    secp256k1_scalar_set_b32(&key, vlckey, NULL);
    int ret = secp256k1_eckey_privkey_serialize(privkey, privkeylen, &key, compressed);
    secp256k1_scalar_clear(&key);
    return ret;
}

int secp256k1_ec_privkey_import(unsigned char *vlckey, const unsigned char *privkey, int privkeylen) {
    DEBUG_CHECK(vlckey != NULL);
    DEBUG_CHECK(privkey != NULL);
    ...
    if (ret)
        secp256k1_scalar_get_b32(vlckey, &key);
    secp256k1_scalar_clear(&key);
    return ret;
}
