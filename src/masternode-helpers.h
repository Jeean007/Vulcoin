...
public:
    ...
    /// Is the inputs associated with this public key? (and there is VLC collateral - checking if valid masternode)
    bool IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey);
    /// Set the private/public key values, returns true if successful
    bool GetKeysFromVlcret(std::string strVlcret, CKey& keyRet, CPubKey& pubkeyRet);
    /// Set the private/public key values, returns true if successful
    bool SetKey(std::string strVlcret, std::string& errorMessage, CKey& key, CPubKey& pubkey);
