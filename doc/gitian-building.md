...
*Setup instructions for a gitian build of Vulcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Vulcoin
...
- [Building Vulcoin](#building-vulcoin)
...
Clone the git repositories for vulcoin and gitian and then checkout the vulcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vulcoin repository
...
To build Vulcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vulcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vulcoin/.git/
...
For example:
```bash
URL=https://github.com/konez2k/vulcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit vulcoin=${COMMIT} --url vulcoin=${URL} ../vulcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vulcoin=${COMMIT} --url vulcoin=${URL} ../vulcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vulcoin=${COMMIT} --url vulcoin=${URL} ../vulcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vulcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vulcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vulcoin-build.assert
```
...
[vulcoin/gitian.sigs](https://github.com/vulcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
